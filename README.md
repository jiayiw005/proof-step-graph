...
- Goal → Tactic edges: a tactic applied to a goal
- Tactic → Goal edges: a new subgoal produced by a tactic
...
Builds the PyPantograph REPL, installs Python dependencies, and fetches the Mathlib cache.
...
uv run python scripts/trace_file.py my_theorems.lean

# Against Mathlib
uv run python scripts/trace_file.py my_theorems.lean --project . --imports Mathlib

# Interactive replay (slower, exact goal IDs)
uv run python scripts/trace_file.py my_theorems.lean --interactive
```
...
```bash
uv run python scripts/trace_dataset.py proofs.jsonl
uv run python scripts/trace_dataset.py proofs.jsonl --project . --timeout 120 --resume
```
...
Walks `.lake/packages/mathlib/Mathlib/**/*.lean` and traces all theorems with server reuse. Requires `lake build` to have run first.

```bash
# Trace a specific Mathlib subpackage
uv run python scripts/trace_mathlib.py --filter Topology
uv run python scripts/trace_mathlib.py --filter Algebra.Group

# Trace all of Mathlib (long)
uv run python scripts/trace_mathlib.py

# Resume after crash, limit to first 100 files
uv run python scripts/trace_mathlib.py --filter Topology --resume --limit 100
```

Output: `data/mathlib_<filter>_graphs.jsonl`

Each entry includes a `source_file` field (e.g., `"Mathlib/Topology/Basic.lean"`) for provenance.
...
2. **Static tracing** (`trace_file.py`, `trace_dataset.py`): sends Lean source through `frontend.process` to get `TacticInvocation` records (before/after goal states per tactic step), then assembles a graph. Goal identity is content-based (hash of goal text).
